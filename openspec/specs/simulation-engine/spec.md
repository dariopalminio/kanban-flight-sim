...
Each eligible workitem SHALL advance with probability `config.advanceProbability` (default 0.5) per tick. The probability is uniform across all levels and states. Eligibility is determined by hierarchical, WIP, and FIFO-queue rules first; probability is applied only to eligible items.
...
- **WHEN** a workitem is not the front-of-queue item in its column (lowest `enteredAt`, tie-broken by array position) during an advancement step
- **THEN** the workitem is not eligible for probabilistic advancement, regardless of hierarchical or WIP rules
